...
namespace FacebookPostBack.BL {
    class Facebook {
        ...
        public string PublishToFacebook(string postText, string pictureURL) {
            try {
                ...
                var rezImage = Task.Run(async () => {
                    using(var http = new HttpClient()) {
                        return await UploadPhotoURL(pictureURL);
                    }
                });
        ...
        public string PublishToFacebook(string postText) {
            try {
                var rezText = Task.Run(async () => {
                    using(var http = new HttpClient()) {
                        return await PublishSimplePost(postText);
                    }
                });
                var rezTextJson = JObject.Parse(rezText.Result.Item2);
                if(rezText.Result.Item1 != 200) {
        ...
        public async Task<Tuple<int, string>> UploadPhotoURL(string photoURL) {
            using(var http = new HttpClient()) {
                var postData = new Dictionary<string, string> {
                { "access_token", _accessToken },
                { "url", photoURL }
